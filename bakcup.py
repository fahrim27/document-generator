...
def download_and_redirect_pdf(pdf):
    time.sleep(10)
    os.remove(pdf)
    return 'success'
...
def submitapi():
    if request.method == 'POST':
        if request.form['key'] == 'vF6DfT5u0VeA8WEZv7RlMDwumlIHOK':
            ...
            name = f"filled_{folder}.docx"
            document = doc.render(newdata)
            doc.save(r'filled_'+folder+'.docx')
...
def downloadpdf():
    if request.method == 'POST':
        if request.form['key'] == 'vF6DfT5u0VeA8WEZv7RlMDwumlIHOK':
            ...
            name = f'filled_'+request.form['pdfname']+'.pdf'
            word = f'filled_'+request.form['pdfname']+'.word'
            
            docpdf = aw.Document(word)
            ...
            threading.Thread(target=download_and_redirect_pdf, args=(name,)).start()
            return send_file(name, as_attachment=True)
        else:
            return {"status": "Credential failed"}
    else:
